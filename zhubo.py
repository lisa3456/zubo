...
# 第二阶段（完全保留，无改动）
def second_stage():
    print("🔔 启动第二阶段：生成 zubo.txt")
    if not os.path.exists(IP_DIR):
        print("⚠️ ip 目录不存在，跳过第二阶段（请先运行fofa_spider.py）")
        return
    combined_lines = []
    ...
    try:
        with open(ZUBO_FILE, "w", encoding="utf-8") as f:
            for line in unique.values():
                f.write(line + "\n")
        print(f"🎯 第二阶段完成，写入 {len(unique)} 条记录到 zubo.txt")
    except Exception as e:
        print(f"❌ 写zubo.txt失败：{e}")

# 第三阶段（完全保留，无改动）
def third_stage():
    print("🧩 启动第三阶段：多线程检测频道 + 生成 IPTV.txt")
    if not os.path.exists(ZUBO_FILE):
        print("⚠️ zubo.txt 不存在，跳过第三阶段（请先确保第二阶段执行成功）")
        return
    ...
    print(f"✅ 线路筛选完成：每个频道最多保留50条，优先北上川IP")
        ...
        try:
            with open(target_file, "w", encoding="utf-8") as wf:
                for ip_p in sorted(ip_set):
                    wf.write(ip_p + "\n")
            print(f"📥 写回可用IP：{target_file}（{len(ip_set)}个）")
        except Exception as e:
                ...
                for ch in ch_list:
                    for line in valid_lines:
                        ...
                        if name == ch:
                            f.write(line + "\n")
                f.write("\n")
        print(f"🎯 IPTV.txt 生成完成，共 {len(valid_lines)} 条有效频道线路")
    except Exception as e:
...
def push_all_files():
    ...
    os.system('git commit -m "自动更新：IP/频道线路" || echo "⚠️ 无文件需提交"')
    os.system("git push origin main || echo '⚠️ GitHub推送失败'")

# 主执行逻辑（无计数、无轮次，启动即执行二、三阶段+推送）
